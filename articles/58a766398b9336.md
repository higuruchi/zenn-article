...
クリーンアーキテクチャのルールを守るために重要な法則である「依存関係逆転の法則」は処理の流れと依存関係を複雑にし、クリーンアーキテクチャを学ぶにあたって難易度が高いと感じたため、自分自身が理解したことを書いていきます。
...
作成したアプリケーションで、依存関係逆転の法則を利用しているプログラムの一例をあげます。
